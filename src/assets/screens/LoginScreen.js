...
const LoginScreen = () => {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            // Using your existing apiCall utility
            const response = await apiCall('POST', '/login', { email, password });
            console.log('Phản hồi từ server:', response);

            if (response.ok) {
                // Assuming your /login endpoint returns data.userId and data.message
                // and you want to pass a username to the Home screen.
                const { message } = response.data;
                const username = email.split('@')[0]; // Still deriving username from email
            ...
            <View style={styles.formContainer}>
              <View style={styles.logoContainer}>
                ...
                <View style={styles.inputWrapper}>
                    <Image
                        source={require('../images/home-icon.png')} // Make sure this path is correct
